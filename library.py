...
def mainscreen():
    ...
    datelabel = Label(screen, text=f"Today's Date: {datetime.date.today()}", font="calibri, 9", width=92, bg="black", fg="white", anchor="n")
...
def mainscreenduebookdisplay():
    ...
    mycursor.execute(f"SELECT title, author, BookID, duedate FROM Borrowedbooks where username = '{username}'")
...
def drawborrowscreen():
    ...
    l2 = Label(screen, text="Title: ", font="calibri, 15")
    l2.place(x=130, y=120)
    objls.append(l2)

    e1 = Entry(screen, textvariable=titleentry, width=50)
    e1.place(x=220, y=125)
    objls.append(e1)

    l3 = Label(screen, text="Author:", font="calibri, 15")
    l3.place(x=130, y=200)
    objls.append(l3)

    e2 = Entry(screen, textvariable=authorentry, width=50)
    e2.place(x=220, y=205)
    objls.append(e2)
    ...
    b2 = Button(screen, text="Borrow Book", font="calibri, 15",
                command=lambda: verifyborrow(titleentry, authorentry, username, bookidentry))
    b2.place(x=260, y=400)
    objls.append(b2)


def verifyborrow(titleentry, authorentry, username, bookidentry):
    title = titleentry.get().upper()
    author = authorentry.get().upper()
    bookid = bookidentry.get().upper()

    searchdetails = ""
    mycursor.execute(
        f"SELECT title, author, BookID FROM availablebooks WHERE title = '{title}' and author = '{author}' and BookID = '{bookid}'")
    for x in mycursor:
        searchdetails = str(x)
    ...
    if len(duelist) >= 1:
        ...
        if len(borrowedlist) >= 5:
            ...
            if title == "" or author == "" or bookid == "":
                tkinter.messagebox.showinfo("ERROR", "Please key in book details!")
                borrowscreen()
            else:
                if searchdetails == f"('{title}', '{author}', '{bookid}')":
                    confirmborrow(title, author, username, bookid)

                else:
                    tkinter.messagebox.showinfo("ERROR", "Book has already been borrowed! Please try again.")
                    borrowscreen()


def confirmborrow(title, author, username, bookid):
...
def drawreturnscreen():
    ...
    b2 = Button(screen, text="Return Book", font="calibri, 15",
                command=lambda: confirmreturn(titleentry, authorentry, bookidentry))
    b2.place(x=260, y=400)
    objls.append(b2)


def confirmreturn(titleentry, authorentry, bookidentry):
    title = titleentry.get().upper()
    author = authorentry.get().upper()
    bookid = bookidentry.get().upper()
    ...
    search = ""
    mycursor.execute(
        f"SELECT username, BookID FROM Borrowedbooks WHERE username = '{username}' and BookID = '{bookid}'")
    for x in mycursor:
        search = str(x)
...
def drawextendscreen():
    ...
    b2 = Button(screen, text="Extend Loan", font="calibri, 15",
                command=lambda: extendbooksearch(titleentry, authorentry, bookidentry, username))
    b2.place(x=260, y=400)
    objls.append(b2)


def extendbooksearch(titleentry, authorentry, bookidentry, username):
    title = titleentry.get().upper()
    author = authorentry.get().upper()
    bookid = bookidentry.get().upper()
    ...
    searchdetails = ""
    mycursor.execute(
        f"SELECT username, author, BookID FROM Borrowedbooks WHERE username = '{username}' and author = '{author}' and BookID = '{bookid}'")
    for x in mycursor:
        searchdetails = str(x)
    ...
    if title == "" or author == "" or bookid == "":
        ...
        if searchdetails == f"('{username}', '{author}', '{bookid}')":
            confirmextend(author, bookid, username)
...
def confirmextend(author, bookid, username):
    mycursor.execute(
...
def getsearch(searchentry):
    ...
    mycursor.execute("SELECT title, author, BookID FROM availablebooks ORDER BY BookID ASC")
    searchresult = mycursor.fetchall()

    finalsearchresult = []  # FILTERED SEARCH RESULT
    ...
    for i in range(len(searchresult)):
        for x in range(len(searchresult[i])):
            if searchtext in searchresult[i][x]:
                if searchresult[i] not in finalsearchresult:
                    finalsearchresult.append(searchresult[i])
